  ...
  for (int i = 0; i < width/CELL_SIZE; i++) {
    for (int j = 0; j < height/CELL_SIZE; j++) {
      for (int k = 0; k < DEPTH/CELL_SIZE; k++) {
        ...
        if ((livingNeighbors >= 2 && livingNeighbors <= ((k*verticalModifier)+abs(closestCenterDistance(cells[i][j][k])*horizontalModifier)))) {
